...
mod tests {
    ...
    impl Fixture {
        ...
        fn with_range(total_pages: usize, manageable_pfns: Range<usize>) -> Self {
            let layout = BuddyAllocator::metadata_layout(total_pages).unwrap();
            let metadata_ptr = NonNull::new(unsafe { alloc(layout) }).expect("test allocation");
            let mut buddy = Box::new(BuddyAllocator::new());
            unsafe {
                buddy
                    .init(metadata_ptr, layout.size(), total_pages, manageable_pfns)
                    .unwrap();
            }
            Self {
                buddy,
    ...
    impl Drop for Fixture {
        fn drop(&mut self) {
            unsafe { dealloc(self.metadata_ptr.as_ptr(), self.layout) };
        }
    }
